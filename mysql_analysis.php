...
EOF;
        ...
        if (isset($param_arr['start-datetime'], $param_arr['stop-datetime']) && $param_arr['start-datetime'] && $param_arr['stop-datetime']) {
            //指定binlog 起始结束时间
            $this->startDatetTime = $param_arr['start-datetime'];
            $this->stopDateTime   = $param_arr['stop-datetime'];
        }

        $this->file = $param_arr['m'] ?? '';
    ...
    public function getBinlog($host, $user, $password, $dataBase, $port)
    {
        ...
        if (!$this->file) {
            ...
            if ($this->mysqli->errno) {
                $this->log($this->mysqli->error, true);
            }
            ...
            if ($info && ($info = $info->fetch_object()) && $binlog && $binInfo = $binlog->fetch_all()) {
                ...
                if (!in_array(strtolower($mode), ['mixed', 'statement'])) {
                    //row模式
            ...
            if (preg_match('/^\\s*(INSERT INTO|UPDATE|DELETE FROM|ALTER TABLE|DROP|CREATE).*/i', $line, $match)) {
                ...
                if ($inTable && !$is_row && !preg_match('/^\\/\\*/', $line, $match)) {
                    ...
                    if (!$is_row && $inTable) {
                        ...
                        if (preg_match(
                            '/(INSERT INTO|UPDATE|DELETE FROM|ALTER TABLE|DROP TABLE IF EXISTS|CREATE TABLE)\\s+([`\\-_a-zA-Z]+\\.[`\\-_a-zA-Z]+)/i',
                            $sql
                        ) || preg_match('/`' . $this->database . '`/', $sql)) {
                            yield 'statement' => $sql;
                        } else {
                            yield 'statement' => preg_replace(
                                '/^(INSERT INTO|UPDATE|DELETE FROM|ALTER TABLE|CREATE TABLE|DROP TABLE IF EXISTS)\\s*([`a-zA-Z_\\-]*)\\s*/i',
                                '$1 ' . $database . '.$2 ',
                                $sql
                            );
                        }
    ...
    public function analysisSql()
    {
        ...
        foreach ($this->dealFile() as $mode => $value) {
            if (preg_match(
                '/(INSERT INTO|UPDATE|DELETE FROM|ALTER TABLE|DROP TABLE IF EXISTS|CREATE TABLE)\\s+([`\\-_a-zA-Z\\.]+)/i',
                $value,
                $match
            )) {
                $table = $match[2];
...
//                    $value = preg_replace("/\s/",' ',$value);
    ...
    public function out($mode, $sql)
    {
        ...
        if (!$this->rollback && $this->rk && preg_match('/^(INSERT INTO)\\s+([_\\-`a-zA-Z\\.]+)/i', $sql, $match)) {
            ...
            if (isset($field_value[0][0], $field_value[0][1])) {
    ...
    public function progress($key)
    {
        ...
        printf(
            "\033[1;33mprogress: [%-50s] %d%% Done;time:%s;sql nums:%d;Memory usage：%s;Memory max usage:%s\r\033[0m",
            str_repeat('>', $key / ($this->lines) * 50),
            $key / ($this->lines) * 100,
            $time,
            $this->sqlNums,
            $kb,
